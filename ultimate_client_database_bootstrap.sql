...
--
-- ┌─────────────────────────────────────────────────────────────────────────┐
-- │  PASTE THIS INTO EVERY NEW ISOLATED CLIENT SUPABASE PROJECT.            │
-- │  Run it ONCE in the SQL Editor of the new project.                      │
-- │  Safe to re-run if it partially fails — all statements are idempotent.  │
-- └─────────────────────────────────────────────────────────────────────────┘
--
-- ─── WHAT THIS DOES ──────────────────────────────────────────────────────────
--   ✅ Creates all tables, indexes, views, functions, triggers
--   ✅ Enables RLS on every table with correct per-role policies
--   ✅ Creates property-photos storage bucket + policies
--   ✅ Seeds a complete demo property with content, pricing defaults,
--      amenities, photos, reviews, FAQs, house rules, policies,
--      local recommendations, email templates, email automations,
--      email settings (disabled), payment settings (manual mode),
--      and account settings (placeholder)
--   ✅ Schedules 3 pg_cron maintenance jobs (requires pg_cron extension)
--
-- ─── WHAT THIS DOES NOT DO ───────────────────────────────────────────────────
--   ✗ Does NOT deploy Edge Functions — must be deployed separately
--   ✗ Does NOT create the admin auth user — create manually in Auth → Users
--   ✗ Does NOT configure Auth URLs — set manually in Auth → URL Configuration
--   ✗ Does NOT add Netlify env vars — set VITE_SUPABASE_URL + VITE_SUPABASE_ANON_KEY
--   ✗ Does NOT configure email/Stripe providers — done via admin panel after launch
--   ✗ Does NOT include platform/SaaS tables (platform_clients, platform_instances, etc.)
--   ✗ Does NOT include real bookings, inquiries, payments, or guest records
--
-- ─── REQUIRED EDGE FUNCTIONS (deploy after running this SQL) ─────────────────
--   Deploy exactly these 16 client Edge Functions:
--
...
--     ical-import
--
--   Do NOT deploy any platform-* Edge Functions into isolated client projects.
--   Platform functions belong only to the Platform Dashboard infrastructure.
--
-- ─── PREFLIGHT: ENABLE THESE EXTENSIONS BEFORE RUNNING ───────────────────────
--   pg_cron   — Project Settings → Extensions → pg_cron
--               Required for cleanup cron jobs. If unavailable, comment out
--               the CREATE EXTENSION and cron.schedule lines in SECTION 12.
--   pgcrypto  — Enabled by default on Supabase. Called below via IF NOT EXISTS.
--   vault     — Enabled by default on Supabase. Required for email/Stripe secrets.
--
-- ─── POST-RUN VERIFICATION ───────────────────────────────────────────────────
--   SELECT id, title, is_active FROM properties;             -- expect 1 row
--   SELECT COUNT(*) FROM bookings;                           -- expect 0
--   SELECT COUNT(*) FROM inquiries;                          -- expect 0
--   SELECT COUNT(*) FROM payment_events;                     -- expect 0
--   SELECT COUNT(*) FROM notification_logs;                  -- expect 0
--   SELECT id, name, public FROM storage.buckets;            -- expect property-photos
--
-- ─── ENV VARS TO COPY AFTER RUNNING ─────────────────────────────────────────
--   VITE_SUPABASE_URL      = https://[new-project-ref].supabase.co
--   VITE_SUPABASE_ANON_KEY = [new project anon key from API settings]
--   (Never reuse the master platform's values here.)
-- =============================================================================
...
-- Date availability overrides (per-date min nights)
...
-- Blocked dates (populated by booking confirmations and iCal imports; starts empty)
...
-- Owner blocks (admin calendar holds)
...
-- Bookings (starts empty — no seed data)
...
-- Public availability view — depends on blocked_dates, owner_blocks, bookings (all created above)
...
-- Payment events (webhook audit log; starts empty)
...
-- payment_settings updated_at trigger (idempotent)
...
-- Notification logs (starts empty)
...
-- Maintenance notes (starts empty)
...
-- Booking internal notes (starts empty)
...
-- Email automation sends log (starts empty)
...
-- Removes inquiries older than 90 days (keeps data available long enough to act on)
...
-- NOTE: Requires pg_cron extension enabled in Project Settings → Extensions
-- If pg_cron is not available, comment out this entire section.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- SECTION 14: SEED — PROPERTY IMAGES (hero + gallery)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- SECTION 22: SEED — EMAIL TEMPLATES (system templates)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- (All seeded as safe defaults — email disabled, payment manual, account blank)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Seeded (demo-ready):
--   properties:               1
--   property_images:          5 hero + ~30 sectioned
--   highlights:               3
--   sleeping_arrangements:    3
--   reviews:                  6
--   amenity_categories:      10
--   amenities:               60
--   property_amenities:      60
--   neighborhood_highlights: 12
--   contact_info:             1
--   photo_sections:          12
--   photo_section_features:  ~50
--   faqs:                    18
--   house_rules:              8
--   property_policies:        4
--   local_recommendations:   13
--   email_templates:         18 (17 system + 1 custom)
--   email_automations:       17
--   email_settings:           1 (provider = disabled)
--   payment_settings:         1 (mode = test_manual)
--   account_settings:         1 (placeholder)
--
-- Empty (awaiting real client use):
--   bookings, inquiries, payment_events, notification_logs,
--   email_automation_sends, blocked_dates, cleaning_tasks,
--   maintenance_notes, booking_internal_notes
--
-- Manual steps still required after running this SQL:
--   1. Enable pg_cron in Project Settings → Extensions (if not already done)
--   2. Configure Auth → URL Configuration (Site URL + Redirect URLs)
--   3. Create admin user in Authentication → Users
--   4. Deploy exactly these 16 client Edge Functions:
--
--      Public — verify_jwt = false:
--        create-booking-request
--        booking-lookup
...
--      Platform functions belong only to the Platform Dashboard infrastructure.
--
--   5. Set Netlify env vars: VITE_SUPABASE_URL + VITE_SUPABASE_ANON_KEY
--   6. Configure email/Stripe via the client admin panel if needed
--   7. Set support contact info: run the Support SQL from the Launch Package page in Platform Admin
-- =============================================================================
